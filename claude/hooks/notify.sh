...
DEBUG_LOG="${CCNOTIFS_TMP}/debug.log"
log() { echo "[$(date +%H:%M:%S)] $*" >> "$DEBUG_LOG"; }

log "--- notify.sh fired: arg=${1:-none} TMUX_PANE=${TMUX_PANE:-UNSET} TMUX=${TMUX:-UNSET}"
...
teleport() {
    log "teleport: bundle=$CCN_TERM_BUNDLE_ID session=$CCN_SESSION win=$CCN_WIN_INDEX pane=$CCN_TMUX_PANE"
    ...
    if [ -n "${CCN_TMUX_BIN:-}" ] && [ -n "${CCN_TMUX_PANE:-}" ] && [ -n "${CCN_SESSION:-}" ] && [ -n "${CCN_WIN_INDEX:-}" ]; then
        # switch-client only works if a client is attached; fall back to select-window + select-pane
        tmux_run switch-client -t "$CCN_SESSION" || true
...
# Resolve the tmux binary that owns the running server.
# The hook's PATH may only have a different (incompatible) version.
TMUX_SERVER_PID=$(echo "$TMUX" | cut -d, -f2)
TMUX_BIN_RESOLVED=""
if [ -n "$TMUX_SERVER_PID" ]; then
    # On macOS, /proc doesn't exist. Use lsof to find the executable.
    TMUX_BIN_RESOLVED=$(lsof -p "$TMUX_SERVER_PID" -Fn 2>/dev/null | grep '/tmux$' | head -1 | sed 's/^n//')
fi
if [ -z "$TMUX_BIN_RESOLVED" ] || [ ! -x "$TMUX_BIN_RESOLVED" ]; then
    TMUX_BIN_RESOLVED=$(command -v tmux 2>/dev/null || echo "tmux")
fi
log "TMUX_BIN_RESOLVED=${TMUX_BIN_RESOLVED}"
...
if [ -n "${TMUX:-}" ] && [ -n "${TMUX_PANE:-}" ]; then
    FOCUS_INFO=$("$TMUX_BIN_RESOLVED" -S "$TMUX_SOCKET" list-panes -a -F '#{pane_id} #{session_attached} #{pane_active} #{window_active}' 2>/dev/null | grep "^${TMUX_PANE} " || echo "")
    SESSION_ATTACHED=$(echo "$FOCUS_INFO" | awk '{print $2}')
    PANE_ACTIVE=$(echo "$FOCUS_INFO" | awk '{print $3}')
    WINDOW_ACTIVE=$(echo "$FOCUS_INFO" | awk '{print $4}')
    : "${SESSION_ATTACHED:=0}" "${PANE_ACTIVE:=0}" "${WINDOW_ACTIVE:=0}"
    log "focus check: attached=$SESSION_ATTACHED pane_active=$PANE_ACTIVE window_active=$WINDOW_ACTIVE"
    if [ "$SESSION_ATTACHED" != "0" ] && [ "$PANE_ACTIVE" = "1" ] && [ "$WINDOW_ACTIVE" = "1" ]; then
        ...
        log "focus check: frontmost=$FRONTMOST_LOWER — skipping notification"
...
TMUX_INFO=""
if [ -n "${TMUX:-}" ] && [ -n "${TMUX_PANE:-}" ]; then
    # Use list-panes which works even when the session has no attached client
    # (display-message requires a client context and returns empty for detached sessions)
    PANE_INFO=$("$TMUX_BIN_RESOLVED" -S "$TMUX_SOCKET" list-panes -a -F '#{pane_id} #{session_name} #{window_index} #{window_name}' 2>/dev/null | grep "^${TMUX_PANE} " || echo "")
    log "list-panes raw for TMUX_PANE=${TMUX_PANE} socket=${TMUX_SOCKET}: [${PANE_INFO}]"
    SESSION=$(echo "$PANE_INFO" | awk '{print $2}')
    WIN_INDEX=$(echo "$PANE_INFO" | awk '{print $3}')
    WINDOW=$(echo "$PANE_INFO" | awk '{print $4}')
    if [ -n "$SESSION" ]; then
        TMUX_INFO="${SESSION}"
    ...
    log "tmux context: SESSION=$SESSION WIN_INDEX=$WIN_INDEX PANE=$TMUX_PANE TMUX_INFO=$TMUX_INFO"
fi
...
log "sending notification: type=$NOTIFY_TYPE title='$TITLE' subtitle='$SUBTITLE' alerter=${ALERTER_BIN:-NONE}"

if [ -n "$ALERTER_BIN" ]; then
    log "launching alerter worker: PANE=${TMUX_PANE:-UNSET} SESSION=${SESSION:-UNSET} WIN=${WIN_INDEX:-UNSET}"
    env \
    ...
    if [ -n "$SUBTITLE" ]; then
        ...
        osascript -e "display notification \"${BODY_ESC}\" with title \"${TITLE_ESC}\" subtitle \"${SUBTITLE_ESC}\""
    else
...
if [ -f "$SOUND_FILE" ]; then
    nohup afplay -v 0.3 "$SOUND_FILE" >/dev/null 2>&1 < /dev/null &
fi
